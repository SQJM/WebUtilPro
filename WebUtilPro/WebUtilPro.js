...
const WebUtilPro = (function () {
  ...
  const Code_Error = {
    ...
    ParameterMismatch: (...arg) => {
      return `CODE Error : <${arg}> Parameter mismatch`;
    },
    // 不存在的项
    NotExistItem: (...arg) => {
      return `CODE Error : <${arg}> Not exist item`;
    },
  }
  ...
  function $(selector, obj = document) {
    ...
    function findElement(selector, context) {
      ...
      if (type === "id") {
        ...
        if (selectorTypes[selector.charAt(1)]) {
          let elements = Array.from((context || document).querySelectorAll("*")).filter((element) => element.parentNode === (context || document));
          let result = [];
          const value = selector.substring(2);
          const valueWithoutBracket = selector.substring(2, selector.length - 1);

          switch (selectorTypes[selector.charAt(1)]) {
            case "class":
              elements.forEach(e => {
            ...
            case "attr":
              {
                const nameAndValue = valueWithoutBracket.split("=");
                if (nameAndValue.length > 1)
                  elements.forEach(e => { if (e.getAttribute(nameAndValue[0]) === nameAndValue[1]) result.push(e) });
                else
                  elements.forEach(e => { if (e.hasAttr(valueWithoutBracket)) result.push(e) });
              }
              break;
            default:
              result = null;
          ...
          const elements = (context || document).querySelectorAll(selector.substring(1));
          return Array.from(elements).filter((element) => element.parentNode === (context || document));
        }
      } else if (type === "soleID") {
        const allElements = (context || document).querySelectorAll("*");
        for (let i = 0; i < allElements.length; i++) {
          if (allElements[i].soleID === selector.substring(1)) {
    ...
    // 提供链式调用支持
    elementObject.$ = function (subSelector) {
      return $(subSelector, this);
    }
  ...
  class Judge {
    ...
    /**
     * 判断给定的参数是否为对象类型包括普通对象、数组、null 等
     * @param {any} obj - 要检查的参数
     * @returns {boolean} 如果参数是对象类型,则返回 true；否则返回 false
     */
    static isObject(obj) {
...
(function () {
  ...
  document.addEvent("mousemove", (event) => {
    WMouseClientX = event.clientX;
    WMouseClientY = event.clientY;
  });
